...
Traditional API gateways are designed for conventional application traffic. They front diverse microservices and primarily manage northbound request handling, authentication, security policy, protocol mediation, and service routing. An AI gateway operates on a different decision surface: it must broker access to heterogeneous LLM providers and self-hosted model stacks, where prompt meaning, data sensitivity, modality, context size, and governance constraints can all affect which model path is legal and useful before inference starts.

ASE therefore treats the AI gateway stack as a two-tier design. Tier one is the provider-facing AI gateway layer. It owns LLM connectivity, API normalization, semantic and policy-aware routing across model families or provider pools, and emission of an explainable `RouteDecision`. Tier two is the self-hosted model-serving layer. It owns inference execution and optimization inside a chosen pool, including engine-specific scheduling, batching, cache locality, and endpoint selection among replicas of the same deployed model. In practice, self-hosted serving systems such as Ollama, vLLM, and SGLang fit this downstream tier-two role even when they also expose gateway-like APIs.

ASE Semantic Router is intentionally a tier-one component. In the inference control plane, semantic routing and load balancing are distinct decisions. Semantic routing selects a legal capability path, model family, or target pool from normalized request context and governance constraints. Load balancing selects a healthy backend endpoint within that pool. ASE Semantic Router therefore operates on request semantics rather than transport metadata and MUST emit an explainable, auditable `RouteDecision` for ASE LLM Load Balancer and any downstream serving stack.

This role boundary should guide future feature placement. Features related to request understanding, provider reachability, policy enforcement, semantic classification, model-family selection, or auditable route explanation belong in ASE Semantic Router. Features related to inference optimization, replica scheduling, KV-cache efficiency, batching, or endpoint-level failover belong in ASE LLM Load Balancer or the downstream tier-two serving layer.

# Background

Current AI-gateway technology is often assembled from two directions. One direction extends traditional API gateways such as Kong AI Gateway or Cloudflare AI Gateway so they can understand LLM traffic and route across providers or model classes. The other direction starts from self-hosted serving systems such as Ollama, vLLM, or SGLang, which are optimized for executing and scheduling inference for specific deployed models. Both directions are useful, but they solve different parts of the stack.

ASE separates those concerns explicitly. The tier-one gateway layer decides whether a request should go to an external provider, an internal provider pool, or a self-hosted model family, and it does so using normalized request context, semantic signals, and governance policy. The tier-two serving layer accepts that higher-level decision and optimizes execution within the selected pool or deployment. That second layer may use engine-native metrics, queue state, batch formation, and cache locality, but those signals are runtime scheduling inputs rather than semantic route inputs.

Many existing LLM routing systems interleave semantic interpretation, policy enforcement, and backend dispatch in one component. That coupling makes route selection harder to explain, govern, and audit, and it also makes it unclear where new features should live. ASE treats semantic routing as a separate request pipeline whose output is a pool-level decision, not an endpoint choice. When evaluating future enhancements, the first question SHOULD be which tier owns the problem: tier-one AI gateway semantics and governance, or tier-two model-serving execution and optimization.
